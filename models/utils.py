...
def get_logger(name: str, run_config: dict) -> logging.Logger:  # pragma: no cover
    ...
    log_save_name = run_config['save']['log_filepath']
    ...
    logger_file = logging.FileHandler(filename=log_save_name)
    logger_file.setFormatter(fmt=formatter)
    logger_file.setLevel(level=getattr(logging, run_config['save']['log_level_file']))
    logger.addHandler(hdlr=logger_file)
    logger_stdout = logging.StreamHandler(sys.stdout)
    logger_stdout.setFormatter(fmt=formatter)
    logger_stdout.setLevel(level=getattr(logging, run_config['save']['log_level_stdout']))
...
def get_cap_override_dict(model_name: str, fixed_caps: dict) -> dict:
    ...
    if len(o_dict) == 0:
        logger.error('Override dict is empty. Check if something has gone wrong.')
...
def get_tech_regions(model: calliope.Model) -> typing.List[typing.Tuple[str]]:
...
def _has_consistent_outputs_1_region(model: calliope.Model) -> bool:  # pragma: no cover
    ...
    # Automatically start debugger if in debug mode and results are inconsistent
    if not passing and 'debug' in config.RUN_ID:
        import pdb
        pdb.set_trace()
